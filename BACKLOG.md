...
## M18 — Screenshots + website redesign

- **Screenshots**: retake all screens after v1.0.34 phone install — Start, Plan, Doing, Waiting, Done tabs, full-screen alarm. Combine into an animated GIF for the website.
- **Website layout**: replace the 3-column persona card section with a 2-column above-the-fold layout: left column = animated GIF of screenshots, right column = app name + tagline + feature bullet list. Rest of page (install steps, about, footer) unchanged.

---

## M19 — App icon rebrand

- Launcher icon and notification icon still use old green/teal assets.
- Generate new icon in periwinkle blue to match the app theme.
- Requires `ic_launcher` and `ic_notification` assets at all densities.

---

## M20 — Mute alarm with lock button

- Pressing the power/lock button while an alarm is sounding should silence the audio but keep the notification and screen visible until explicitly dismissed or snoozed.
- Relevant API: `AudioManager` focus + `KeyEvent.KEYCODE_POWER` in `ReminderActivity`.

---

## M21 — Full schedule specification for weekly/monthly/yearly tasks
...
- **What to cover**: the 5 tabs and their purpose (Plan = define, Start = act, Doing = in progress, Waiting = blocked, Done = today's wins); the ▶ / ↻ icons on Start cards; long-press to delete in Plan.
...
- **Countdown reminders before a fixed event**: daily reminders starting N days before a fixed event. Design: `countdownDays` field on fixed tasks; scheduler generates a daily alarm series.
- **Snooze UX revisit**: replace fixed-duration snooze buttons with a "remind me in X" input (user-specified duration or time).
- **HomeViewModel tick optimization**: replace `while(true)/delay(60s)` with a `tickerFlow` combined via `SharingStarted.WhileSubscribed`.
- iOS app
- Cloud sync / backup
